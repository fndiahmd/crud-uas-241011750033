...
<section class="bg-gradient-to-br from-emerald-700 to-emerald-900 pt-28 pb-16">
    <div class="max-w-7xl mx-auto px-4">
        <div class="flex items-center gap-2 text-emerald-200 text-sm mb-3"><a href="{{ route('home') }}" class="hover:text-white">Home</a><span>/</span><a href="{{ route('kategori') }}" class="hover:text-white">Kategori</a><span>/</span><span class="text-white">{{ $kategori->nama_kategori }}</span></div>
        <h1 class="text-4xl md:text-5xl font-bold text-white">{{ $kategori->nama_kategori }}</h1>
        <p class="text-emerald-200 mt-2 text-lg">{{ $produks->total() }} produk ditemukan dalam kategori ini.</p>
    </div>
</section>
<div class="max-w-7xl mx-auto px-4 py-10">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div class="col-span-full text-center py-16">
            ...
            <p class="text-stone-400 text-lg">Belum ada produk di kategori ini.</p>
            <a href="{{ route('produk') }}" class="inline-block mt-4 text-emerald-600 font-semibold hover:underline">Jelajahi semua produk →</a>
        </div>
        @endforelse
    </div>
